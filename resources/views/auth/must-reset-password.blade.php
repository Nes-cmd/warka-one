<x-guest-layout>
    <div class="text-center mb-8">
        <h1 class="text-3xl font-bold text-gray-900 dark:text-white mb-2">Password Reset Required</h1>
        <p class="text-gray-600 dark:text-gray-400">You must reset your password before continuing</p>
    </div>

    <!-- Session Status -->
    <x-auth-session-status class="mb-4" :status="session('status')" />

    <form method="POST" action="{{ route('password.must-reset.store') }}" class="space-y-6">
        @csrf
        ...
        <div class="space-y-2">
            ...
            <div class="relative">
                <x-text-input id="current_password" class="block w-full dark:text-white pr-12" type="password" 
                              name="current_password" required autocomplete="current-password" placeholder="Enter your current password" />
                ...
                <button type="button" class="absolute inset-y-0 right-0 px-4 flex items-center text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 focus:outline-none" 
                        onclick="togglePasswordVisibility('current_password', 'toggleCurrentPasswordIcon')">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" id="toggleCurrentPasswordIcon">
