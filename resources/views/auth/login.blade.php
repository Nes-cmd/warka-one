<x-guest-layout>
    ...
    <form @submit="isLoading = true" method="POST" action="{{ route('login') }}" class="mt-8" x-data="{ isLoading: false }">
        ...
        <div x-data="{
            ...
            <div class="border border-radius-2 rounded flex justify-around py-2 mb-4 dark:border-gray-600">
                <button type="button" :class="authwith == 'email'?'bg-secondary-50 text-primary border-b-2 border-secondary':'bg-gray-100 text-gray-800'" class="w-[40%] py-2 rounded" x-on:click="() => {authwith = 'email'}">Email</button>
                <button type="button" :class="authwith == 'phone'?'bg-secondary-50 text-primary border-b-2 border-secondary':'bg-gray-100 text-gray-800'" class=" w-[40%] py-2 rounded" x-on:click="() => {authwith = 'phone'}">Phone</button>
            </div>
            @endif
            ...
            <div class="mt-4 relative">
                ...
                <x-text-input id="passwordInput" class="dark:text-white block mt-1 w-full" type="password" name="password" required autocomplete="current-password" />
