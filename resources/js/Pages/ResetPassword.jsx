...
export default function ResetPassword({ authflowData, errors: propErrors }) {
    ...
    useEffect(() => {
        if (!authflowData || !authflowData.authwith) {
            router.visit(route('v2.authflow.get-otp') + '?for=reset-password');
        }
    }, [authflowData]);
        ...
        <GuestLayout title="Reset Your Password">
            ...
            {propErrors?.general && (
                <div className="mb-6 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 text-red-700 dark:text-red-300 px-4 py-3 rounded-lg">
                    <p className="text-sm font-medium">{propErrors.general}</p>
                </div>
            )}
            ...
            <form onSubmit={submit} className="space-y-6">
                ...
                <div className="pt-4">
                    <button
                        type="submit"
                        disabled={processing}
                        className={`w-full py-3 flex items-center justify-center text-lg font-semibold rounded-lg transition-all duration-200 transform hover:scale-105 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500 disabled:cursor-not-allowed disabled:transform-none ${
                            processing
